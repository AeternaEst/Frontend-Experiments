...
export default class PropertyService {
  getProperties = (): Promise<Property[]> => new Promise((resolve) => {
    setTimeout(() => {
  ...
  addToFavorite = (propertyId: number): Promise<void> => new Promise((resolve, reject) => {
    setTimeout(() => {
   ...
   Promise<void> => new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (propertyToAddComment) {
        propertyToAddComment.comments = [...propertyToAddComment.comments, comment];
        resolve();
      }
      reject(new Error('Property not found'));
    }, 2000);
